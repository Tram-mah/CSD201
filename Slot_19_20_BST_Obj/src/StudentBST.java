...
public class StudentBST {
    ...
    private Node insertRec(Node p, Student st) {
        if (p == null) {
            p = new Node(st);
        } else if (st.compareTo(p.info) < 0) {   //st.getId()<p.info.getId()||st.compareTo(p.info) < 0
    ...
    private Student search(Node p, int id) {
        if (p == null) {
            return null;
        }
        if (p.info.getId() == id) {
    ...
    private int countNodes(Node p) {
        ...
        return 1 + countNodes(p.left) + countNodes(p.right);// nhớ phải cộng 1
    }
